...
// - המחיר המודפס הוא מחירון או מחיר מבצע שכבר ניתן; אחוזי ההנחה הקבועה חבויים,
...
חוק העל — שער יחיד:
סכום הטענות חייב לסגור בדיוק את הפער שמוצג לך בכותרת "הפער להסבר" — גם בכסף (לאגורה) וגם ביחידות כאשר נמסר פער יחידות. אסור להמציא כסף ואסור להמציא יחידות. חריג יחיד לכסף: "נטו לחיוב" של ברמן מעוגל אצל הספק שורה-שורה, ולכן שארית של עד 15 אגורות בין סכום הטענות לפער — כשהטענות עצמן סוגרות את פער היחידות במדויק וכל מספריהן מהקלט — אינה כישלון: החזר את הטענות וציין את שארית העיגול ב-unexplained. מעבר לכך, אם ההסבר אינו נסגר — החזר claims כמערך ריק, ורשום ב-unexplained מה חסר כדי לסגור. הסבר חלקי שנראה סביר גרוע מהודאה שאין הסבר.

שש מוסכמות הנייר של ברמן:
1. על תעודת המשלוח מודפס מחיר מחירון או מחיר מבצע שכבר ניתן, ללא סכום שורה מודפס. אחוז ההנחה הקבוע של החנות אינו מודפס — הוא מופעל במרכזת החיוב. לכן המחירים והסכומים שנמסרים לך בקלט כבר חושבו לפי המחיר המוסכם נטו של כל מוצר (מחירון פחות ההנחה הקבועה). השתמש בהם כפי שנמסרו.
2. מבצע של ברמן הוא מחיר קנייה קבוע שמחליף את המחיר המוסכם של המוצר בתקופת המבצע, ואינו מצטבר עם ההנחה הקבועה. לפעמים המבצע כבר ניתן בתעודה ולפעמים הוא יקוזז רק במרכזת; זה יכול להשתנות בין שורות באותה תעודה ובין תעודות. בטענת חוסר או עודף, amountExVat מחושב לפי המחיר שנמסר לך לאותו מוצר בקלט (מחיר השורה בתעודה כפי שחושב). אל תחליף אותו במחיר אחר ואל תחיל עליו הנחה נוספת.
...
רקע על הנייר של ברמן (תעודות אמיתיות שנבדקו): זהו סרט הדפסה צר של מדפסת שטח.
בראשו פרטי המאפייה ("י. את א. ברמן בע"מ / מאפיית ברמן"), סוכן, מסלול, תאריך ושעה;
אחריו כותרת סוג המסמך ("תעודת משלוח") ומתחתיה מספר התעודה; אחריו פרטי הלקוח
("לכבוד", "לקוח:", כתובת, "תאריך יום עבודה"); ואז טבלת הפריטים.
עמודות הטבלה מימין לשמאל: מספר שורה | ברקוד | פריט | תיאור פריט | כמות | מחיר | סכום.
עמודת "סכום" מודפסת ככותרת אך התאים שלה ריקים — רק קו תחתון בכל שורה. אל תמציא לה ערכים.
הברקוד הוא קטע מספרי פנימי (לרוב 6–7 ספרות), וקוד הפריט ("פריט") הוא 1–6 ספרות.
המחיר המודפס ליחידה לפני מע"מ יכול להיות מחיר מחירון רגיל או מחיר מבצע שכבר ניתן.
הבחירה היא לכל שורה בנפרד: באותה תעודה חלק מהמוצרים יכולים להופיע במחיר מבצע
ואחרים במחירון, וגם אותו מוצר יכול להגיע במחירים שונים בתעודות שונות.
הנחות החנות הקבועות אינן מודפסות. מחיר מבצע שכבר מודפס הוא מחיר סופי בפני עצמו.
העתק את המחיר כפי שמודפס, בלי להפעיל הנחה ובלי לבחור מחיר כדי לסגור את הסכום.
אין לנסות לסגור כסף מול סכומי הסיכום — הכסף נבדק בלקוח שמכיר את ההנחות והמבצעים.
מתחת לטבלה מודפס בלוק סיכום, בסדר הזה: "סהכ שורות N" (מונה שורות המוצר);
"נטו לחיוב" עם הסכום לפני מע"מ; שורת "מעמ" עם האחוז והסכום; "סה הכל תעודה לחיוב"
(הסכום כולל מע"מ); ואז בלוק תחתון עם "ביקורת:", "נומרטור", "סה"כ כללי: N.00"
(סך היחידות בתעודה, מודפס עם שתי ספרות עשרוניות), "שיגור", "חתימת לקוח" ו"מספר תעודה פנימי".
על השורות יש לעיתים סימוני עט של העובד (וי, נקודות צבעוניות) — הם אינם חלק מהנתונים;
התעלם מהם, ואם סימון מסתיר ספרה שאינך מצליח לקרוא בוודאות — החזר null והוסף אזהרה.
מספרי השורות המודפסים יכולים לדלג (למשל שורה 13 חסרה בין 12 ל-14) — זה תקין בנייר של ברמן.
קיימות גם תעודות החזרה ("ח.משלוח החזרה" בכותרת) שבהן החנות מחזירה סחורה למאפייה,
...
6. quantity היא הכמות המודפסת בשורה. unitPriceExVat הוא המחיר המודפס בעמודת המחיר — מחירון או מחיר מבצע, לפי אותה שורה. החזר אותו כפי שהוא מודפס; אל תנסה להוזיל, לעגל או להתאים אותו לשום סכום אחר.
...
9. docType הוא "credit" אם המסמך הוא תעודת החזרה (כותרת עם "החזרה", למשל "ח.משלוח החזרה") או תעודת/חשבונית זיכוי — כל מסמך שמחזיר סחורה או כסף למאפייה; "invoice" לתעודת משלוח או חשבונית רגילה; ו-"unknown" רק אם הכותרת אינה קריאה. הסיווג הזה נקבע לפי הכותרת בלבד — לא לפי סימן, צבע או תוכן השורות. בכל סוגי המסמכים החזר את הכמויות והסכומים בדיוק כפי שהם מודפסים על הנייר, כערכים חיוביים; אל תוסיף מינוס משלך ואל תהפוך סימן. הסימן (חיוב מול זיכוי) נגזר באפליקציה מ-docType, ולכן מספר שתחזיר עם מינוס ייספר פעמיים לרעה.
...
פריסת הטבלה משתנה בין מכתבים. שתי פריסות שנצפו בפועל, מימין לשמאל:
- ברקוד (עמודה ימנית ללא כותרת) | "קוד מוצר" (1–6 ספרות) | "שם המוצר" | "מחיר קניה ללא מע"מ (בזמן המבצע)" | "מחיר לצרכן מומלץ עד"
- "ברקוד" (עם כותרת) | "שם המוצר" | "מחיר קניה ללא מע"מ (בזמן המבצע)" | "מחיר לצרכן מומלץ עד" — בלי עמודת קוד מוצר כלל
קרא תמיד לפי הכותרות המודפסות בפועל. הברקוד הוא 5–13 ספרות. מכתב בלי עמודת קוד מוצר
הוא תקין לחלוטין — החזר בו itemCode: null לכל הפריטים, בלי אזהרה.

כללים מחייבים:
1. קבע תחילה את כיוון הקריאה הנכון של כל תמונה (0°/90°/180°/270°) וקרא לפי הכיוון הנכון.
2. חלץ כל מוצר במבצע בדיוק פעם אחת, לפי שורות הטבלה. שם מוצר יכול להישבר לשתי שורות מודפסות בתוך אותו תא — זו שורת מוצר אחת, לא שתיים. כותרות, הערות שוליים וטקסט שיווקי אינם פריטים.
3. itemCode ו-barcode הם ספרות בלבד, בדיוק כפי שמודפס, כל אחד מהעמודה שלו — אל תחליף ביניהם (כשקיימות שתי העמודות, הברקוד הוא הארוך מבין השניים). ספרה לא קריאה — null באותו שדה ואזהרה. אסור להשלים או לתקן ספרות.
4. name הוא שם המוצר כפי שמודפס במכתב, גם אם מקוצר (למשל "10 לחמנ' עננים בריוש").
5. buyPriceExVat הוא מחיר הקנייה ללא מע"מ בזמן המבצע, כפי שמודפס — ייתכן מספר שלם (10) או עם ספרה עשרונית אחת (8.2). consumerPrice הוא "מחיר לצרכן מומלץ עד" כפי שמודפס. אל תחשב אחד מהשני ואל תמיר מע"מ.
6. תקופת התוקף מודפסת בדרך כלל פעם אחת מתחת לטבלה עבור כל המכתב, בנוסחים כמו "המבצע בתוקף מ- 1/7-31/8/26" או "המבצע בתוקף מ- 01/03/2026 ועד 30/04/2026" — החזר אותה לכל פריט. אם מודפס תוקף שונה לשורה מסוימת — הוא גובר עבורה.
...
8. אל תגזור ערך חסר מערכים אחרים ואל תמציא פריטים. שדה שאינו מודפס — null, עם אזהרה אם הוא מהותי (למעט itemCode במכתב שאין בו עמודת קוד מוצר — שם null הוא המצב התקין).
...
export function scanChecksumMismatches(scan, inputDocuments) {
  ...
  for (const doc of (scan && scan.documents) || []) {
    ...
    // לכן רק הלקוח יכול לבדוק כסף. שדה lineTotalExVat שאינו קיים גרם
    // כאן לסכום אפס ולקריאה חוזרת מיותרת גם בפענוח מושלם.
  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      for (const document of documents) {
        ...
        // עוגן היחידות: "סה"כ כללי" המוקלד. חיובי תמיד — גם בתעודת זיכוי מוקלד
        // המספר המודפס על הנייר; הסימן נגזר באפליקציה מסיווג המסמך.
